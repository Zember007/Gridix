...
type OneSignalWebhookBody = Record<string, unknown>;
...
function extractMessageId(body: OneSignalWebhookBody): string | null {
...
function extractEventType(body: OneSignalWebhookBody): string {
  const candidates = [body["event"], body["type"], body["event_type"]];
  for (const c of candidates) {
    if (typeof c === "string" && c.trim()) return c.trim();
  }
  return "unknown";
...
serve(async (req) => {
  ...
  try {
    ...
    const body = (await req.json().catch(() => null)) as OneSignalWebhookBody | null;
    ...
    if (onesignalMessageId) {
      const { data: job, error: jobErr } = await supabaseAdmin
        ...
        .select("id")
        .eq("onesignal_message_id", onesignalMessageId)
        .maybeSingle();
    ...
    if (insertErr) {
      ...
      return createJsonResponse({ success: false, error: insertErr.message }, 500, origin);
    }
