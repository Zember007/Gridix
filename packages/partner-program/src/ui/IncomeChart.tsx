...
export const IncomeChart: React.FC<IncomeChartProps> = ({ data, dates, language }) => {
    ...
    const points = useMemo(() => {
        return data.map((val, i) => {
            ...
            const y = height - ((val - minVal) / (maxVal - minVal || 1)) * (height - padding * 2) - padding;
    ...
    return (
        ...
        >
            ...
            {activePoint && (
                <div 
                    className="absolute z-10 bg-slate-900 text-white px-3 py-2 rounded-lg shadow-xl text-xs pointer-events-none transform -translate-x-1/2 -translate-y-full transition-all duration-75"
                    style={{ 
                ...
                >
                    <div className="font-bold text-sm">${activePoint.val}</div>
                    <div className="text-slate-400 whitespace-nowrap">{activePoint.date ? new Date(activePoint.date as string).toLocaleDateString(language === 'ru'? 'ru-RU' : 'en-US', { day: 'numeric', month: 'long' }) : ''}</div>
