...
type UsertourClient = {
  ...
  start: (
    contentId: string,
    options?: { once?: boolean; continue?: boolean },
  ) => Promise<void> | void;
  reset: () => void;
...
const USERTOUR_CONTENT_IDS: {
  ru: UsertourContentIds;
  nonRu: UsertourContentIds;
} = {
  ru: {
 ...
 */
export async function identifyUsertourUser(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
...
export async function startProjectCreationTour(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
...
export async function startProjectEditorTour(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
...
export async function startPartnersTour(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
...
export async function startAdminChecklist(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
...
export async function startProjectChecklist(
  payload: IdentifyPayload,
): Promise<void> {
  if (!isBrowser()) return;
