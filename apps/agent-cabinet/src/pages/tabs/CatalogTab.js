...
export function CatalogTab() {
    ...
    return (_jsxs("div", { className: "flex flex-col h-full overflow-hidden bg-[#F8FAFC]", children: [selectedProject && (_jsx(SharedProjectDrawer, { project: drawerProject ?? toSharedProject(selectedProject), mode: "agent", 
                /*  onLock={(project) => {}} */
                onClose: handleCloseDrawer, onShare: handleShareProject, onOpenPublicPage: handleOpenPublicPage, t: (key, params) => t(`common.${key}`, params), renderUnitsTab: (p) => _jsx(AgentUnitsTab, { project: p }) })), _jsx(ModuleHeader, { title: t("common.catalog.title"), subtitle: selected
