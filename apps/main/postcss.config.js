  ...
  return {
    ...
    Once(root) {
      ...
      rtlRoot.walkAtRules((atRule) => {
        ...
        if (atRule.name === "charset" || atRule.name === "import") atRule.remove()
      })
      ...
      rtlRoot.walkRules((rule) => {
        ...
        rule.walkDecls((decl) => {
          if (decl.prop === "left" && isFixedValue(decl.value)) hasLeftFixed = true
          if (decl.prop === "right" && isFixedValue(decl.value)) hasRightFixed = true
          if (decl.prop === "margin-left") hasMarginLeft = true
          if (decl.prop === "margin-right") hasMarginRight = true
        })
        if (hasLeftFixed && !hasRightFixed) rule.append({ prop: "right", value: "auto" })
        if (hasRightFixed && !hasLeftFixed) rule.append({ prop: "left", value: "auto" })
        // margin: в RTL не должны накладываться ml и mr — сбрасываем противоположную сторону в 0
        if (hasMarginLeft && !hasMarginRight) rule.append({ prop: "margin-right", value: "0" })
        if (hasMarginRight && !hasMarginLeft) rule.append({ prop: "margin-left", value: "0" })
      })
