...
function selectorUIReducer(
  state: SelectorUIState,
  action: SelectorUIAction,
): SelectorUIState {
  switch (action.type) {
...
export const useSelectorUIState = () => {
  ...
  const openApartmentModal = useCallback(
    (apartment: Apartment) =>
      dispatch({ type: "OPEN_APARTMENT_MODAL", apartment }),
    [],
  );
  ...
  const openApartmentDetails = useCallback(
    (apartment: Apartment) =>
      dispatch({ type: "OPEN_APARTMENT_DETAILS", apartment }),
    [],
  );
  ...
  const openSidePanel = useCallback(
    (panelState: SidePanelState) =>
      dispatch({ type: "OPEN_SIDE_PANEL", state: panelState }),
    [],
  );
