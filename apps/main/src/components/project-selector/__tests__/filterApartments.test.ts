...
describe("filterApartments", () => {
  ...
  it("filters by rooms (free_layout)", () => {
    const result = filterApartments(
      apartments,
      { ...DEFAULT_STATE, selectedRooms: "free_layout" },
      "USD",
    );
    expect(result.map((a) => a.id)).toEqual(["4"]);
  });
  ...
  it("filters by type", () => {
    const result = filterApartments(
      apartments,
      { ...DEFAULT_STATE, selectedType: "parking" },
      "USD",
    );
  ...
  it("filters by showOnlyAvailable", () => {
    const result = filterApartments(
      apartments,
      { ...DEFAULT_STATE, showOnlyAvailable: true },
      "USD",
    );
