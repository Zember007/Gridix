...
const ProjectsGallery = ({
  ...
  const processProjects = (rawProjects: BaseProject[]) => {
    const processedProjects: Project[] = rawProjects.map(
      (project: BaseProject) => {
  ...
  return (
    <div className={`w-full ${embedMode ? "" : "mx-auto max-w-7xl"}`}>
      {showHeader && (
          ...
          <div className="mb-6 rounded-lg bg-white p-6 shadow-sm">
            <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label>
                  {t("gallery.priceRange")}: {formatPrice(priceRange[0] ?? 0)} -{" "}
                  {formatPrice(priceRange[1] ?? 10000000)}
                </Label>
                <Slider
                  value={priceRange}
            ...
            <div className="flex items-center justify-between border-t pt-4">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <span>
                  {t("gallery.foundCount")}: {filteredProjects.length}
                </span>
                <Button variant="outline" size="sm">
      ...
      {/* Сетка проектов */}
      <div
        className={`grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 ${embedMode ? "p-4" : ""}`}
      >
            ...
            <CardContent className="p-4">
              ...
              <div className="mb-3 flex items-center gap-4 text-sm text-gray-600">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>
                    {project.apartment_count} {t("gallery.apartments")}
                  </span>
                </div>
              </div>
      ...
      {filteredProjects.length === 0 && (
        <div className="py-12 text-center">
          ...
          <p className="text-sm text-gray-400">
            {t("gallery.changeSearchCriteria")}
          </p>
        </div>
      )}
    </div>
