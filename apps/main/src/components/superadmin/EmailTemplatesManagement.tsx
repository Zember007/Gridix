...
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@gridix/ui";
...
import { Loader2, Plus, Edit, Trash2, Save, Eye, EyeOff, Code, LayoutTemplate } from 'lucide-react';
...
export const EmailTemplatesManagement = () => {
    ...
    const [newTemplateHtml, setNewTemplateHtml] = useState('<html><body><h1>Hello!</h1></body></html>');
    ...
    const currentTemplate = templates.find(t => t.key === selectedKey && t.locale === activeLocale);
    ...
    const handleDeleteTemplate = async (id: number) => {
        if (!confirm('Are you sure you want to delete this specific locale template?')) return;
    ...
    return (
        <div className="space-y-6 p-6 h-full flex flex-col">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight text-foreground">Email Templates</h2>
                    <p className="text-muted-foreground">Manage your transactional email templates</p>
                </div>
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                        <Button onClick={resetForm} className="gap-2 shadow-sm hover:shadow-md transition-all">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden max-h-[calc(100vh-12rem)]">
                ...
                <div className="w-1/3 min-w-[300px] flex flex-col gap-4 overflow-y-auto pr-2">
                    {uniqueKeys.map((key) => {
                        // Find the 'en' version or first available for preview info
                        const previewTemplate = templates.find(t => t.key === key && t.locale === 'en') || templates.find(t => t.key === key);
                        if (!previewTemplate) return null;

                        return (
                            <Card
                                key={key}
                                className={`cursor-pointer transition-all hover:shadow-md border-l-4 ${selectedKey === key
                                    ? 'border-l-primary shadow-md bg-accent/10'
                                    : 'border-l-transparent hover:border-l-primary/50'
                                    }`}
                                onClick={() => {
                                    setSelectedKey(key);
                                    // Switch to 'en' or first available if current activeLocale doesn't exist for this key
                                    const hasCurrentLocale = templates.some(t => t.key === key && t.locale === activeLocale);
                                    if (!hasCurrentLocale) {
                                ...
                                <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{previewTemplate.subject_template}</p>
                                        </div>
                ...
                {/* Editor Area */}
                <div className="flex-1 flex flex-col bg-background border rounded-xl shadow-sm overflow-hidden">
                    {selectedKey ? (
                        <>
                            {/* Editor Header & Tabs */}
                            <div className="border-b bg-muted/30">
                                <div className="p-4 pb-2 flex justify-between items-start">
                                    <div className="flex flex-col gap-1">
                                        <h3 className="font-medium text-lg">{selectedKey}</h3>
                                        {currentTemplate && (
                                            <Badge variant={currentTemplate.is_active ? 'default' : 'secondary'} className={`w-fit ${currentTemplate.is_active ? 'bg-green-600' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {currentTemplate && (
                                            <Button
                                                ...
                                                className="h-8"
                                            >
                                                <Trash2 className="h-4 w-4 mr-2" /> Delete {activeLocale}
                                            </Button>
                                        )}
                                        <Button
                                            variant="default"
                                ...
                                <div className="px-4 flex items-center gap-2">
                                    <Tabs value={availableLocalesForKey.includes(activeLocale) ? activeLocale : availableLocalesForKey[0]} onValueChange={setActiveLocale} className="w-full">
                                            ...
                                            <TabsList className="bg-transparent p-0 h-auto justify-start border-none">
                                                {availableLocalesForKey.map(loc => (
                                                    <TabsTrigger
                                                        key={loc}
                                                        value={loc}
                                                        className="data-[state=active]:bg-background data-[state=active]:shadow-sm border border-transparent data-[state=active]:border-border rounded-t-md rounded-b-none px-4 py-2"
                                                    >
                                                        {loc.toUpperCase()}
                                                    </TabsTrigger>
                                                ))}
                                            ...
                                            <Dialog open={isAddLocaleDialogOpen} onOpenChange={setIsAddLocaleDialogOpen}>
                                                ...
                                                <DialogContent className="sm:max-w-[425px]">
                                                    <DialogHeader>
                                                        ...
                                                        <DialogDescription>
                                                            Add a new translation for <strong>{selectedKey}</strong>. Content will be copied from the current template.
                                                        </DialogDescription>
                                                    </DialogHeader>
                                                    <div className="grid gap-4 py-4">
                                                        <div className="grid grid-cols-4 items-center gap-4">
                                                            <Label htmlFor="new-locale" className="text-right">
                                                                Locale Code
                                                            </Label>
                                                            <Input
                                                                id="new-locale"
                                                    ...
                                                    <DialogFooter>
                                                        <Button type="submit" onClick={handleAddLocale}>Add Locale</Button>
                                                    </DialogFooter>
                                                </DialogContent>
                                            </Dialog>
                                        </div>
                                ...
                                <>
                                    <div className="p-4 space-y-4 border-b">
                                        <div className="grid gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => updateLocalTemplate({ subject_template: e.target.value })}
                                                className="font-medium"
                                            />
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                <Label htmlFor="active-toggle" className="cursor-pointer">Active Status</Label>
                                                <Switch
                                                    id="active-toggle"
                                                    checked={currentTemplate.is_active}
                                                    onCheckedChange={(c) => updateLocalTemplate({ is_active: c })}
                                                />
                                            </div>
                                            ...
                                            <div className="flex bg-muted p-1 rounded-lg">
                                                ...
                                                >
                                                    <LayoutTemplate className="h-3 w-3 mr-1" /> Split
                                                </Button>
                                                <Button
                                    ...
                                    <div className="flex-1 overflow-hidden relative flex">
                                        {(viewMode === 'code' || viewMode === 'split') && (
                                            <div className={`${viewMode === 'split' ? 'w-1/2 border-r' : 'w-full'} flex flex-col h-full bg-slate-950`}>
                                                <div className="bg-slate-900 text-slate-400 px-4 py-1 text-xs font-mono border-b border-slate-800">HTML Source</div>
                                                <Textarea
                                                    value={currentTemplate.html_template}
                                                    onChange={(e) => updateLocalTemplate({ html_template: e.target.value })}
                                                    className="flex-1 p-4 font-mono text-sm leading-relaxed bg-transparent border-none text-slate-200 resize-none focus-visible:ring-0 rounded-none h-full"
                                                    spellCheck={false}
                                                />
                                            </div>
                                        )}
                                        ...
                                        {(viewMode === 'preview' || viewMode === 'split') && (
                                            <div className={`${viewMode === 'split' ? 'w-1/2' : 'w-full'} flex flex-col h-full bg-white`}>
                                                <div className="bg-slate-100 text-slate-500 px-4 py-1 text-xs font-medium border-b flex justify-between">
                                                    <span>Preview</span>
                                                    <span className="text-[10px]">Locale: {activeLocale}</span>
                                                </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8">
                            ...
                            <h3 className="text-xl font-medium mb-2">Select a template key to edit</h3>
                            <p className="max-w-md text-center">
                                Choose a template group from the list on the left to manage its translations and configuration.
                            </p>
                        </div>
                    )}
                </div>
