        ...
        <div className="flex flex-col items-start justify-between gap-6 lg:flex-row lg:items-center">
          ...
          <div className="mt-2 flex w-full flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:gap-4 lg:mt-0 lg:w-auto">
            <button
              ...
              className="flex-1 rounded-lg border border-gray-200 px-6 py-3 text-center text-sm font-bold uppercase tracking-wider text-gray-500 transition-colors hover:border-gray-200 hover:bg-gray-100 hover:text-gray-800 sm:border-transparent lg:flex-none"
            >
        ...
        <div className="flex w-full flex-col items-center gap-3 md:w-auto md:flex-row">
          ...
          <div className="flex w-full items-center gap-2 md:w-auto">
            <div className="group relative flex-1 md:flex-none">
              <Calendar
                size={16}
                className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 transition-colors group-hover:text-gray-600"
              />
              <Input
                type="date"
                value={startDate}
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="w-40 px-6 py-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
                    {t("partners.transactionsDate")}
                  </th>
                  <th className="w-32 px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">
                ...
                {filteredTransactions.map((tx, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`whitespace-nowrap px-6 py-4 text-right text-sm font-bold ${sumStyles}`}
                      >
                        {tx.sum > 0 ? `+ $${tx.sum}` : `- $${Math.abs(tx.sum)}`}
                      </td>
                      <td className="whitespace-nowrap px-6 py-4 text-right font-mono text-sm text-gray-500">
                      ...
                      <td className="relative px-4 py-4 text-center">
                        ...
                        {openMenuIndex === idx && (
                          <div
                            ref={menuRef}
                            className="absolute right-8 top-1/2 z-20 w-48 origin-right -translate-y-1/2 rounded-lg border border-gray-100 bg-white shadow-xl duration-100 animate-in fade-in zoom-in-95"
